...
List<Card> cards = [
  ...
  Card('Magierin', CardType.Wizard, false, 5, (deck) {}, (deck) {
    return amountOf(activeDeck(deck), {CardType.Land, CardType.Flood, CardType.Weather, CardType.Flame}) * 5;
  }),
...
class Deck extends State<FantastischeReiche> {
      ...
      body: Center(
        child: ListView.builder(
          ...
          itemBuilder: (context, i) {
            Card card = _deck.keys.elementAt(i);
            return ListTile(
              title: Row(
                mainAxisAlignment: MainAxisAlignment.spaceBetween,
                children: [
                  Container(
                      ...
                      decoration: BoxDecoration(
                        boxShadow: [BoxShadow(color: card._cardType.color, spreadRadius: 10)],
                        borderRadius: BorderRadius.circular(16),
                        color: card._cardType.color,
                      ),
                      child: Text(
                        '${card._baseStrength}',
                        style: TextStyle(
                          color: card._cardType == CardType.Army ? Colors.white : Colors.black,
                        ),
                      )),
                  Center(
                    child: Text(card._name,
                        style: TextStyle(
                            decoration:
                                _deck[card] ?? true ? TextDecoration.none : TextDecoration.lineThrough)),
                  ),
                  Text('${card._bonus(_deck)}')
                ],
              ),
  ...
  void _addCard(String cardName) {
    setState(() {
      Card card = cards.firstWhere((element) => element._name == cardName);
      if (!_deck.containsKey(card)) {
        _deck[card] = null;
        _calculateDeck();
      }
    });
  ...
  void _calculateDeck() {
    setState(() {
      ...
      _sum = _deck.keys
          .where((e) => _deck[e] == null || _deck[e]!)
          .fold(0, (previousValue, element) => previousValue + element.calculateStrength(_deck));
    });
  }

  int maxCards() {
    if (_deck.keys.map((e) => e._name).contains('Totenbeschwörer')) return 8;
    return 7;
  }
}
                ...
                children: [
                  Flexible(
                    child: Container(
                      margin: const EdgeInsets.only(right: 15),
                      decoration: BoxDecoration(
                        boxShadow: [BoxShadow(color: cards[i]._cardType.color, spreadRadius: 10)],
                        borderRadius: BorderRadius.circular(16),
                        color: cards[i]._cardType.color,
                      ),
                      child: Text('${cards[i]._baseStrength}',
                          style: TextStyle(
                            color: cards[i]._cardType == CardType.Army ? Colors.white : Colors.black,
                          )),
                    ),
                  ),
                  Expanded(
                    child: Align(
                      ...
                      child: Text(cards[i]._name),
                    ),
                  )
                ],
              ),
              onTap: () {
                Navigator.pop(context, cards[i]._name);
              },
            );
          }),
    );
